...
public struct BottomSheet<Content: View>: View {
    ...
    @EnvironmentObject private var keyboardObservable: KeyboardObservable
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: -8) {
                        //Spacer()
                        self.content.padding(.bottom, geometry.safeAreaInsets.bottom - keyboardObservable.currentHeight)
                        Spacer()
                    }
                }
                ...
                .offset(y: self.isPresented ? (geometry.size.height/2 - self.height/2 + geometry.safeAreaInsets.bottom + self.draggedOffset - keyboardObservable.currentHeight) : (geometry.size.height/2 + self.height/2 + geometry.safeAreaInsets.bottom))
            }
        }
    }
